...
double ph2T_reg5(double p, double h)
{
   ...
   f = h - pT2h_reg5(p, T1);
   if (fabs(f) > FN_TOLERANCE)
   {
      if (f > 0)
         T2 = TMAX5;
      else
         T2 = TMIN5;
      T = rtsec(pT2h_reg5, p, h, T1, T2, FIRST_FIXED, MAX_ITER, CONVERGENCE_PRECISION);
   }
   else
      T = T1;
   if (T < TMIN5)
...
double ps2T_reg5(double p, double s)
{
   ...
   if (fabs(f) > FN_TOLERANCE)
   {
      if (f > 0)
         ...
         T2 = TMIN5;
      T = rtsec(pT2s_reg5, p, s, T1, T2, FIRST_FIXED, MAX_ITER, CONVERGENCE_PRECISION);
   }
   else
      T = T1;
   if (T < TMIN5)
...
double hs2p_reg5(double h, double s)
{
   ...
   if (fabs(f) > FN_TOLERANCE)
   {
      if (f > 0)
         ...
         p2 = PMIN5;
      p = rtsec(ph2s_reg5, h, s, p1, p2, SECOND_FIXED, MAX_ITER, CONVERGENCE_PRECISION);
   }
   else
      p = p1;
   return (p);
