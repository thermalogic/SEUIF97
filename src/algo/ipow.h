...
__attribute__((always_inline))static inline double ipowrqm(double x, int i)
{
        double ans = 1.0;
        if (i < 0) {
            x = 1.0 / x;
...
__attribute__((always_inline)) static inline double possac(double x, int n)
{
...
__attribute__((always_inline)) static inline double ipowsac(double x, int n)
{
	if (n >= 0)
	{
		return possac(x, n);
