...
// 1,2,3,4 区 smin ->smax。5区另外处理
{
  ...
  double hmin = 2500.89261782; // pT2h_reg1(Pmin, 273.15);
  // Right point in h-s plot
  double hmax = 4160.70777426; // pT2h_reg2(Pmin, 1073.15);
  double smax = 11.92105507;   // pT2s_reg2(Pmin, 1073.15);

  // Region 4 left and right point
  double h4l = 0.0;           // pT2h_reg1(Pmin, 273.15);
  double s4l = 0.0;           // pT2s_reg1(Pmin, 273.15);
  double h4v = 2500.89261782; // pT2h_reg2(Pmin, 273.15);
  double s4v = 9.15575940;    // pT2s_reg2(Pmin, 273.15);
  // !!!! Check region 5 MUST BE On TOP !!!
  // if （s4v <= s && s<= smax） (h,s)may be setup to error region2
  double s_r5_1 = 6.51708829;    // pT2s_reg5(50.0, 1073.15);
  double s_r5_2 = 13.90495608;   // pT2s_reg5(P_MIN, 2273.15);
  double h_r5_1 = 3926.05014007; // pT2h_reg5(50.0, 1073.15);
  double h_r5_2 = 7376.98026360; // pT2h_reg5(P_MIN, 2273.1
  ...
  if (5.049096828 <= s && s < 5.260578707) {
    ...
    double hmin = h4l + (s - s4l) / (s4v - s4l) * (h4v - h4l);
    double hs = _h2c3b_s(s);
    double h23max = 2812.94206060; // pT2h_reg2(100, 863.15);
    double h23min = 2563.59200389; // pT2h_reg2(Ps_623, 623.15);
    if (hmin <= h && h < hs)
      return 4;
    if (hs <= h && h < h23min)
      return 3;
...
int Th_region(double T, double h) {
  ...
  if (((T > TMIN3) && (T <= tc_water) &&
       (((h > hss) && (h < hB23)) || ((h >= hp100) && (h < hsw)))) ||
